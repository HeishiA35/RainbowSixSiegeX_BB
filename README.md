...
### オペレータプールの変更
+ ノールの追加
+ ドッケビのEMPインパクトグレネードを削除、ブリーチングチャージ追加
...
# UPDATE 2026/4/16(Thu)
## Y11S1.2.1アップデート対応
### マッププールの変更
+ 運河、高層ビル、テーマパーク、アウトバック有効化

# UPDATE 2026/4/15(Wed)
## Y11S1.2のアップデート対応
### オペレータプールの変更
+ ジャッカルにフラググレネードを追加
+ スモークに展開型シールドを追加
...
### マッププールの変更
+ エメラルドプレーンズ有効化
+ 運河、アウトバック、高層ビル、テーマパーク無効化
### オペレータプール変更
+ ソリッドスネーク追加
## 技術アップデート
+ Javascriptをリファクタリング
+ PCでCANVAS上のボタンUIを小さく表示。
+ オペレータアイコンを修正。canvas上の視認性を向上。
+ オペレータボタンに閉じるボタンを追加
+ 名前付きマップの実装(settingモーダルで切り替え)
+ スタンプサイズの変更を実装(settingモーダルで切り替え)
## バグ修正
+ ヴィラのマップを修正(公式配布マップの誤り)

# UPDATE 2026/1/27(tue)
## 技術アップデート
...
## 運用
### ランクマッププールの変更
+ 公式サイトからイメージ画像やblueprintをimageディレクトリ内に格納し、map_pool.jsでファイルパスを指定。(既存のマップであれば有効化するなど適宜対応)
+ index.htmlおよびpage-board.htmlのマッププールに関する構成を変更。

### オペレータの追加
+ 公式サイトからアイコン画像やアビリティimageディレクトリ内に格納し、operator_pool.jsでファイルパスを指定。
